...
int main(){
    string s="(7+9)*4/8-3"; // called string infix exprassion
    ...
    */
    ...
    for(int i=0;i<s.size();i++){
        ...
        if(s[i]>=48 && s[i] <=57){
            ...
            if(operant.size()==0)  operant.push(s[i]);
            else if(s[i]=='(') operant.push(s[i]);
            else if(operant.top()=='(') operant.push(s[i]);
            else if(s[i]==')'){
                while (operant.top()!='(')  
                {
                        char ch=operant.top();
                    ...
                    operant.pop(); // opening bracket ko bhi uda do
            }
            else if(priority(s[i])>priority(operant.top())) operant.push(s[i]);
